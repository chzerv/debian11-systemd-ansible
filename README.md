# Debian 11 (Bullseye) Container Image for Ansible Testing

This Dockerfile builds a Debian 11 (Bullseye) based container, capable of using `systemd`, mainly for Ansible role testing.
...
2. Clone the repository: 
   ...
   ```shell
   git clone https://github.com/chzerv/debian11-systemd-ansible.git
   ```
3. `cd` into the directory and run `docker build -t debian11-systemd-ansible .`
...
2. Pull this image from _Docker hub_:
    ...
    ```shell
    docker pull chzerv/debian11-systemd-ansible:latest
    ```
...
```shell
docker run -d --privileged --volume=/sys/fs/cgroup:/sys/fs/cgroup:ro debian11-systemd-ansible:latest ansible --version
```
...
platforms:
  - name: instance
    image: "chzerv/${IMAGE:-debian11}-systemd-ansible:latest"
